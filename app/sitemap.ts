...
const technologyRoutesEn = technologies.map(
  (tech) => `https://www.devslearning.com/technologies/${tech.slug}/courses`
);
const technologyRoutesEs = technologies.map(
  (tech) => `https://www.devslearning.com/es/technologies/${tech.slug}/courses`
);

const courses: ITechnology[] = await fetchCourses();
const coursesRoutesEn = courses.map(
  (course) => `https://www.devslearning.com/courses/${course._id}/`
);
const coursesRoutesEs = courses.map(
